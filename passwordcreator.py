...
def reclaclulate_password_callback():
	for row in public_globals.gridarray:
		...
		if int(PasswordLengthString)>32:
			tkinter.messagebox.showinfo("Error!", "Error: password cannot be longer than 32 characters long, defaulting to " + str(public_globals.PasswordLength))
...
def callback_save():
	...
	result = tkinter.messagebox.askquestion("Save sites list", "Are You Sure you want to save the file " + public_globals.FileName + " and overwrite it?", icon='warning')
	if result == 'yes':
		print("Saving...")
...
def callback_load():
	FileName_TMP = tkinter.filedialog.askopenfile(mode='r')
	if not FileName_TMP==None: #filename will be None when no file is selected	
		...
		try:
			...
			e = sys.exc_info()[0]
			tkinter.messagebox.showinfo("Error!", "Error: %s, if this error is value error, you are either opening an empty file or an invalid file" % e)
...
def create_printable_file_callback():
	...
	file_.close()
	tkinter.messagebox.showinfo("Warning!", "Warning, READ CAREFULLY: we have generated the file report.txt, print it before clicking ok, after you click ok, the file will be deleted for security purposes")
